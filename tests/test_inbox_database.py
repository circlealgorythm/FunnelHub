    ...
    async with async_session_maker() as session:
        ...
        session.add(
            MessengerIdentity(
                ...
                lead_id=lead.id,
                channel="telegram",
                external_user_id="db-tg" if gc_id == TEST_GC_ID else f"db-tg-{gc_id}",
                username="database_tg" if gc_id == TEST_GC_ID else f"database_tg_{gc_id}",
                is_subscribed=True,
                raw_profile={},
            )
        )
        session.add(
            MessengerIdentity(
                ...
                lead_id=lead.id,
                channel="vk",
                external_user_id=str(199271782 + (gc_id - TEST_GC_ID)),
                username=None,
                display_name=None,
                is_subscribed=True,
                raw_profile={},

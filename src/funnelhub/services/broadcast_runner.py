...
async def _ensure_conversation(session: AsyncSession, lead_id: uuid.UUID, channel: str) -> Conversation:
        ...
        for channel in broadcast.channels:
            try:
                ...
                if channel in ("telegram", "vk"):
                    ...
                    if not identity or not identity.is_subscribed:
                ...
                if channel == "telegram" and clients.telegram_bot:
                    res_tg = await send_telegram_text_message(session=session, bot=clients.telegram_bot, lead_id=target.lead_id, text=broadcast.message_text)
                    message_id = res_tg.message_id
                elif channel == "vk" and clients.vk_client:
                    res_vk = await send_vk_text_message(session=session, client=clients.vk_client, lead_id=target.lead_id, text=broadcast.message_text)
                ...
                logger.warning(f"Broadcast {broadcast.id} failed to send to lead {target.lead_id} via {channel}: {e}")

        if success_any:
            target.status = "sent"
            broadcast.processed_leads += 1
        else:
            if all("unsubscribed" in msg or "not linked" in msg or "no email" in msg for msg in error_messages):

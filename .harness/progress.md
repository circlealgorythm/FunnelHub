...
- Email funnel buttons can use internal `funnelhub://bot/telegram` and `funnelhub://bot/vk` URLs. The runner resolves them at send time through the lead's active bot-link token with the same Telegram/VK builders used by the thank-you page; VK uses OAuth when configured and falls back to `vk.me`.

...
- `GETCOURSE_WEBHOOK_SECRET_REQUIRED=true` is still not enabled. Next step is to add `fh_secret=<production secret>` to the real `aisukam.ru`/GetCourse redirect or configure GetCourse server-to-server webhook with the `X-FunnelHub-Webhook-Secret` header, then test a real application before strict mode.

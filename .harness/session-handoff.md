...
- Do not enable `GETCOURSE_WEBHOOK_SECRET_REQUIRED=true` yet if the real secret must be passed in query params. Current Uvicorn access logs include full query strings, so a query secret would appear in production logs. Header transport is preferred for `/webhooks/getcourse`; `/join/getcourse` browser redirects need access-log redaction/disablement or another transport decision before strict query-secret enforcement.
